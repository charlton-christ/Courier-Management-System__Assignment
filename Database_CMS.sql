...
SELECT Employee_ID, COUNT(*) AS [Number of Couriers] FROM Courier_Details GROUP BY Employee_ID 

--15. Calculate the total revenue generated by each location
SELECT LocationID, SUM(Amount) AS TotalRevenue FROM Payment_Details GROUP BY LocationID 

--16. Find the total number of couriers delivered to each location.
SELECT LocationID, COUNT(*) AS [Total Couriers Delivered] FROM Payment_Details GROUP BY LocationID

--17. Find the courier with the highest average delivery time:
SELECT TOP 1 a.Courier_ID,AVG((DATEDIFF(DAY, a.Delivery_Date,b.Payment_Date))) AS [Avg. Delivery Time] FROM Courier_Details a LEFT JOIN Payment_Details b ON a.Courier_ID = b.CourierID GROUP BY a.Courier_ID ORDER BY AVG((DATEDIFF(day, a.DeliveryDate,b.Payment_Date)))DESC

--18. Find Locations with Total Payments Less Than a Certain Amount
SELECT LocationID, SUM(Amount) AS TotalPayments FROM Payment_Details GROUP BY LocationID HAVING SUM(Amount) < 50000

--19. Calculate Total Payments per Location
SELECT SUM(Amount) AS [Total Payments] FROM Payment_Details GROUP BY LocationID

--20. Retrieve couriers who have received payments totaling more than $1000 in a specific location (LocationID = X):
SELECT CourierID FROM Payment_Details WHERE LocationID = 42 GROUP BY CourierID HAVING SUM(Amount) > 10000

--21. Retrieve couriers who have received payments totaling more than $1000 after a certain date (PaymentDate > 'YYYY-MM-DD'):
SELECT CourierID FROM Payment_Details WHERE CAST(Payment_Date AS DATE) > '2024-02-28' GROUP BY CourierID HAVING SUM(Amount) > 10000

--22. Retrieve locations where the total amount received is more than $5000 before a certain date (PaymentDate > 'YYYY-MM-DD')
SELECT LocationID FROM Payment_Details WHERE CAST(Payment_Date AS DATE) > '2024-03-01' GROUP BY LocationID HAVING SUM(Amonut)>50000
